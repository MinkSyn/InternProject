...
from general import average_KNN, all_KNN, load_model, change_type
...
def run(
    ...
    source = str(source)
    resnet, emb_face = load_model(KNN=KNN)#model)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                # Write results
                for *point, conf, cls in reversed(det):
                    x1, y1, x2, y2 = point
                    w, h = int(point[2]-point[0]), int(point[3]-point[1])
                    x, y = int((point[0]+point[2])/2 - w/2), int((point[1]+point[3])/2 - h/2)
                    
                    if KNN == 'all':
                        result, distance = all_KNN(im0[y:y+h,x:x+w], model=resnet, emb_face=emb_face, threshold=0.6)
                    elif KNN == 'center':
                        result, distance = average_KNN(im0[y:y+h,x:x+w], model=resnet, emb_face=emb_face, threshold=0.6)
                    ...
                    if c == 0:
                        ...
                        label = 'Incorrect'
                    annotator.box_label(point, label, color=colors(c, True))
