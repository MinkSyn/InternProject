...
class YOLOV5():
	''' YOLOV5 load file detect_image từ model YoloV5 để tách và phân loại đối tượng
...
class FaceNet():
	...
	def parameter_person(self):
		# Lấy các đặc trưng của từng object trong file person và save vào Model_Parameter/FaceNet/Face_data.pt
		...
		# torch.save(data, 'Model_Parameter/FaceNet/Face_data.pt') # saving Face_data.pt file
	...
	def face_match(self, face): # face= image of object face, type là np.array 
		...
		return (self.name_list[idx_min], d)
...
def identify_images(path_dict, path_images_test, path_faceNet, view_result, save_image):
	yolo = YOLOV5(
		path=path_dict,
		source=path_images_test)
	fNet = FaceNet(
		path=path_faceNet)

	data = yolo.take_object()
	...
	for path in paths:
		...
		for i in range(n):
			...
			result = fNet.face_match(img_crop) # Nhận kết quả
			...
			if view_result or save_image:
				...
				cv2.putText(im0, result[0],(x,y+h+int(h*0.15)), fontFace=cv2.FONT_HERSHEY_COMPLEX, fontScale=size, color=colors[name_class])
...
if __name__ == '__main__':
	...
	identify_images(
		...
		path_faceNet='Model_Parameter/FaceNet/Face_data.pt', # Đường dẫn file model đã train sẵn để nhận diện khuôn mặt
		view_result=True, # Demo kết quả
		save_image=True
		)