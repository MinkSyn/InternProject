        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    if path in dict_data.keys():
                        dict_data[path].append([names[int(cls)], xyxy])
                    else:
                        dict_data[path] = [[names[int(cls)], xyxy]]
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default='yolov5/data/facemask.yaml', help='(optional) dataset.yaml path')
